...
# /wait
	...
	while [[ -z $EXIT ]] ; do
		...
		echo -e "$URL\n 💤	...\n$ACCESS"
		SRC=$(w3m -debug -dump_source -o http_proxy="http://$(cat bestproxy.txt)" $ENC $URL/altars/?close=reward -o user_agent="$(shuf -n1 .ua)")
		SRC=$(w3m -debug -dump_source -o http_proxy="http://$(cat bestproxy.txt)" $ENC $URL/altars/enterFight -o user_agent="$(shuf -n1 .ua)")
...
# /view
	w3m -debug -o http_proxy="http://$(cat bestproxy.txt)" $ENC $URL/altars -o user_agent="$(shuf -n1 .ua)" | head -n15 | tail -n14 | sed "/\[user\]/d;/\[arrow\]/d;/\ \[/d" | grep --color $ACC
	echo "Altars (✔)"
	sleep 30
}
