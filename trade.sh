...
_built () {
	if [[ -n $CLD ]] ; then
		...
		w3m -debug -dump "$URL/clan/$CLD/built/?goldUpgrade=true&r=$CODE" -o user_agent="$(shuf -n1 .ua)"
		CODE=$(w3m -debug -dump_source -o accept_encoding=='*;q=0' $URL/arena/quit -o user_agent="$(shuf -n1 .ua)" | sed "s/href='/\n/g" | grep "attack/1" | head -n1 | cut -d\/ -f5 | tr -cd "[[:digit:]]")
		echo -e "/clan/$CLD/built/?silverUpgrade=true&r=$CODE"
		w3m -debug -dump "$URL/clan/$CLD/built/?silverUpgrade=true&r=$CODE" -o user_agent="$(shuf -n1 .ua)"
