...
# /wait
	...
	while [[ -z $EXIT ]] ; do
		...
		echo -e " 💤	...\n$ACCESS"
		SRC=$(w3m -dump_source -o http_proxy="http://$proxy" $ENC "$URL$ACCESS" -o user_agent="$(shuf -n1 .ua)")
		ACCESS=$(echo $SRC | sed 's/href=/\n/g' | grep '/king/' | head -n1 | awk -F\' '{ print $2 }')
		EXIT=$(echo $SRC | grep -o 'king/kingatk/')
	done
# /game
...
# /kingatk
		if [[ -n $KINGATK && $ddg -ne 4 && $hl -ne 18 && $HP2 -gt 15 ]] ; then
			echo '👑'
			SRC=$(w3m -debug -dump_source -o http_proxy="http://$proxy" $ENC "$URL$KINGATK" -o user_agent="$(shuf -n1 .ua)")
			_access
			sleep $ITVL
			ddg=$[$ddg+1]
			hl=$[$hl+1]
...
# /view
	echo ""
	w3m -debug -o http_proxy="http://$proxy" $ENC $URL/king -o user_agent="$(shuf -n1 .ua)" | head -n15 | tail -n14 | sed "/\[user\]/d;/\[arrow\]/d;/\ \[/d" | grep --color $ACC
	echo "King (✔)"
	sleep 30
}
