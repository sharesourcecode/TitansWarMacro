...
# /enterFight
	...
	echo -e "\nClan fight"
	echo $URL
	SRC=$(w3m -debug -dump_source -o http_proxy="http://$(cat bestproxy.txt)" $ENC $URL/clanfight/?close=reward -o user_agent="$(shuf -n1 .ua)")
...
# /wait
	...
	while [[ -z $EXIT ]] ; do
		...
		echo -e " 💤	...\n$ACCESS"
		SRC=$(w3m -debug -dump_source -o http_proxy="http://$(cat bestproxy.txt)" $ENC "$URL/clanfight" -o user_agent="$(shuf -n1 .ua)")
...
# /view
	echo ""
	w3m -debug -o http_proxy="http://$(cat bestproxy.txt)" $ENC $URL/clanfight -o user_agent="$(shuf -n1 .ua)" | head -n15 | tail -n14 | sed "/\[user\]/d;/\[arrow\]/d;/\ \[/d" | grep --color $ACC
	echo "ClanFight(✔)"
	sleep 30
}
