...
_career () {
	...
	if [[ -n $CLD ]]; then
		echo "Clan ID: $CLD"
		w3m -debug -o accept_encoding=='*;q=0' "$URL/clan/$CLD/quest/take/6" -o user_agent="$(shuf -n1 .ua)" | head -n15 &
		w3m -debug -o accept_encoding=='*;q=0' "$URL/clan/$CLD/quest/help/6" -o user_agent="$(shuf -n1 .ua)" | head -n15 &
	fi
